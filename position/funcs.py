...
def calSatPos(data, timeCor=False, iteration='Newton'):
    ...
    for j in range(data.shape[0]):
        ...
        v = 2*np.arctan(np.sqrt((1+e)/(1-e))*np.tan(E/2))#np.arctan2(np.sqrt(1-np.power(e,2))*np.sin(E),np.cos(E)-e)
        phi = v + w
        ...
        sats[j,0] = x1*np.cos(Omega) - y1*np.cos(i)*np.sin(Omega)
        sats[j,1] = x1*np.sin(Omega) + y1*np.cos(i)*np.cos(Omega)
        sats[j,2] = y1*np.sin(i)
    return sats
